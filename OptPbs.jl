...
# non convex problems of dimension > 1
...
function OptPbs()
    solvers = Dict{Symbol, Function}(:CR => LCR,:CG => LCG)
...
p = performance_profile(hcat([p[:, 1] for p in values(stats)]...),
                            ...
                            title=string("Performance profile : #f in the resolution of ", pb_type),
...
p = performance_profile(hcat([p[:, 2] for p in values(stats)]...),
                            collect(String, [string(s) for s in keys(stats)]),
                            title=string("Performance profile : #g in the resolution of ", pb_type),
...
p = performance_profile(hcat([p[:, 3] for p in values(stats)]...),
                            collect(String, [string(s) for s in keys(stats)]),
                            title=string("Performance profile : #Hv in the resolution of ", pb_type),
...
p = performance_profile(hcat([sum(p, 2) for p in values(stats)]...),
                            collect(String, [string(s) for s in keys(stats)]),
                            title=string("Performance profile : #f + #g  + #Hv in the resolution of ", pb_type),
